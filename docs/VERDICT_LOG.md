...
| 29-EUR | 2026-06-18 | **EURUSD BOS-retest continuation** (Trial 29) — 4H bias + 1H BOS close above/below confirmed swing → last opposite-body OB before impulse → retest OB close → entry next bar open. SL=OB-boundary±0.1%, TP=2R fixed. Both sides. Pre-registered gate: n≥50 AND net PF>1.0 at BOTH standard and 2× cost stress. Runner: `--signal continuation --symbol-preset EURUSD`. 5yr holdout (31120×1H bars). | 4H+1H | 335 | 0.8270 | 0.0792 | 0.7359 (std) / 0.6573 (2×) | 29.2% | **FAIL** — Gross PF 0.83 (no raw edge before fees). Win rate 29% = BTC EMA dead-zone. Both cost levels fail. Do not tune. |
| 29-GBP | 2026-06-18 | **GBPUSD BOS-retest continuation** (Trial 29) — identical chain/exit to 29-EUR, GBPUSD spread 1.0pip+0.6pip rt. 5yr holdout. | 4H+1H | 339 | 1.1535 | 0.0759 | 1.0315 (std) / 0.9254 (2×) | 36.6% | **FAIL at 2× stress** — Standard cost barely passes (1.03) but 2× stress fails (0.93). Gross PF=1.15 marginal. Fails pre-registered dual-level gate. **HALT-AND-ESCALATE: Both Forex SMC signal families (reversal-chain + continuation) exhausted without robust PASS. Forex SMC track closed.** |
| ST-1 | 2026-06-18 | **Session Trader trial 1** — IB sweep + CHoCH entry on EURUSD/GBPUSD. Initial Balance = first 2H of London (08-10 UTC) and NY (13-15 UTC). Entry: current-bar close after CHoCH (close on far side of IB extreme). SL = sweep wick ± 3 pip buffer. TP = IB range × 1.5 (London) / 2.0 (NY) projected, fallback 3R. TP1 = 1R. 4H macro bias filter. Cost: VT Markets forex (EURUSD 1.4pip rt, GBPUSD 1.6pip rt). 5yr holdout (31120×1H). Runner: `scripts/backtest_session.py`. | 4H+1H | EUR:34 GBP:38 combined:72 | EUR:0.26 GBP:0.22 | — | EUR:0.24 GBP:0.21 | EUR:12% GBP:11% | **FAIL** — Low win rate (11%) + large R from post-CHoCH entry (SL anchored at sweep wick far below entry). avg_r = −0.57. Root cause: entry at current-bar close is far past the CHoCH inflection; SL remains at deep sweep wick → R too large, TP rarely reached. Next trial: retest entry at broken IB level (smaller R). Do not tune this config. |
